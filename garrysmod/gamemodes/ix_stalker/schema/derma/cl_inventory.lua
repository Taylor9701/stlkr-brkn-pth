...
vgui.Register("ixDragImage", PANEL, "DImage")
...
vgui.Register("ixItemImage", PANEL, "DImage")
...
function PANEL:AddIconImg(image, x, y, w, h, skin) 	-- Lack of preview on this is because only panel is getting dragged
	local iconSize = self.iconSize					-- and I haven't found a way to get around that without massive
													-- efforts, so I won't. Live with it.
	w = w or 1
	h = h or 1

	if (self.slots[x] and self.slots[x][y]) then
		local mat = Material(image)
		local iconWidth = w * iconSize
		local iconHeight = h * iconSize
		
		local panel = self:Add("ixItemImage")
		panel:SetSize(iconWidth, iconHeight)
		panel:SetZPos(20)
		panel:InvalidateLayout(true)
		panel:SetPos(self.slots[x][y]:GetPos())
		panel.gridX = x
		...
		local itemPic = panel:Add("ixDragImage")
		itemPic:SetImage(image)
		itemPic:SetKeepAspect(true)
		
		if (w > h) then
			local ratio = (mat:Width() / mat:Height())		-- get ratio'd nerd
			local newWidth = (iconHeight * ratio)
			itemPic:SetSize(newWidth, iconHeight)
		else
			local ratio = (mat:Height() / mat:Width())
			local newHeight = (iconWidth * ratio)
			itemPic:SetSize(iconWidth, newHeight)
		end
		
		itemPic:Center()
		itemPic:SetZPos(21)
		itemPic:InvalidateLayout(true)
